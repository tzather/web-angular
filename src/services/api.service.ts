...
export class ApiService {
   ...
   */
  getJson = (url: string, interval?: number, errorMessage?: string) => this.get("application/json", url, interval, errorMessage).pipe(res => res, shareReplay());

  getCsv = (url: string, interval?: number, errorMessage?: string) => this.get("text/csv", url, interval, errorMessage).pipe(res => res, shareReplay());
   ...
   * @returns json object if succesful, otherwise null
   */
  post = async (url: string, model: any, errorMessage?: string): Promise<any> => await this.fetchApi("post", url, "application/json", errorMessage, model);
   ...
   * @returns json object if succesful, otherwise null
   */
  put = async (url: string, model: any, errorMessage?: string): Promise<any> => await this.fetchApi("put", url, "application/json", errorMessage, model);
  ...
  private get(contentType: string, url: string, interval?: number, errorMessage?: string): Observable<any> {
    return new Observable<any>(subscriber => {
      ...
      if (interval && interval > 0) { // if interval is set then call it 
        // setInterval(async () => {
  ...
  private async fetchApi(method: string, url: string, contentType: string, errorMessage?: string, model?: any): Promise<any> {
    ...
    return fetch(url, {
      ...
      switch (contentType) {
        ...
        case "text/csv":
          return response.text().then((m) => m.trim().split("\n").map(v => v.split(",")));
        default:
